...
# En caso de tener valores atípicos se puede hacer una ANOVA robusta usando el paquete WRS2
...
# En caso de que no sean normales se puede aplicar la prueba de Kruskal Wallis (no parametrica)

# homocedasticidad
plot(fit, 1)
# En el gráfico no se observan relaciones evidentes entre los residuos y valores ajustados(medias de cada grupo). Se asume homogeneidad de varianzas

# Se puede hacer lo mismo a través de una prueba de Levene
PlantGrowth %>% 
  levene_test(weight ~ group)
# El valor de p = 0.341 no es significativo. No hay diferencias significativas entre las variaciones de los grupos. Se asume la homogeneidad de los grupos. 
...
# El efecto es pequeño 0.264 (26%) significa que el 26% del cambio de peso puede explicarse por las condiciones del tratamiento

# Pruebas post hoc para anova unidireccional significativo
# Prueba de Tukey hace comparaciones por pares entre grupos
PlantGrowth %>% 
  pairwise_t_test(weight ~ group, p.adjust.method = "bonferroni", pool.sd = TRUE)
